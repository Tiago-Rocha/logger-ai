...
| **MVP Layer 3: Background Upload** | Scheduled upload | Works in background | `UploadSchedule` + `configureScheduling` |
|  | Cancellation | Host can disable work | `cancelScheduling()` |
|  | Retry & backoff | Network resilience | Delegate-driven policy in UploadManager |
...
| **Optional Layer 8: Developer Hooks** | Success/failure delegate | Host visibility | `LoggerDelegate` callbacks |
