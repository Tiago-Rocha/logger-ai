...
For the Flutter SDK, run `dart test` inside `sdk/dart` for unit/integration work and `dart run tests/acceptance/bin/main.dart` for the acceptance DSL harness. While we focus on ATDD, `./scripts/test-flutter.sh` executes only the acceptance scenarios and produces merged coverage in `coverage/lcov.info`.
